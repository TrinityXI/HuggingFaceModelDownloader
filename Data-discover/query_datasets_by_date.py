...
class HuggingFaceDatasetQuery:
    ...
    def get_datasets_by_date(
        ...
        print(f"获取到 {len(datasets)} 条数据集记录")
        
        # 过滤出指定日期的数据集
        result = []
        for dataset in datasets:
            # 解析时间字段
            if date_field not in dataset:
                continue
            
            try:
                # 时间字段可能是 ISO 8601 格式的字符串
                date_str = dataset[date_field]
                if isinstance(date_str, str):
                    ...
                    try:
                        ...
                        # 如果 fromisoformat 失败，尝试其他格式
                        # 例如: "2024-01-15T10:30:00" (没有时区信息)
                        if 'T' in date_str and '+' not in date_str and 'Z' not in date_str:
                            # 假设为 UTC
                            parsed_date = datetime.fromisoformat(date_str + '+00:00')
                        else:
                            raise
                elif isinstance(date_str, (int, float)):
                    # 如果是时间戳（秒或毫秒）
                    if date_str > 1e10:  # 毫秒时间戳
                        parsed_date = datetime.fromtimestamp(date_str / 1000, tz=timezone.utc)
                    else:  # 秒时间戳
                        parsed_date = datetime.fromtimestamp(date_str, tz=timezone.utc)
                else:
                    continue
                
                # 如果 parsed_date 没有时区信息，假设为 UTC
                ...
                # start_time 和 end_time 已经是 UTC（在 get_datasets_by_date 中设置）
                # 检查是否在目标日期范围内
                if start_time <= parsed_date < end_time:
                    result.append(dataset)
                elif parsed_date < start_time:
                    # 由于是按时间降序排列，如果遇到更早的数据，可以提前结束
                    break
