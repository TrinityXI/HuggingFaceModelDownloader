...
你的可用工具如下：
1. scan_dataset：扫描和搜索 Hugging Face 数据集 (支持关键词、时间、下载量、标签过滤)
2. query_dataset：查询指定数据集的下载状态 (支持进度和事件日志)
3. download_dataset：触发指定数据集的下载 (支持优先级和强制重试)
4. tar_dataset：对已下载完成的数据集进行压缩打包 (支持压缩、分片、删除源文件)
